...
**Как это работает:**
Возьмем число `123`.
Мы хотим сдвинуться влево и добавить `4`, чтобы получилось `234`.
Нам не нужно читать `2`, потом `3`, потом `4`.
Мы делаем так:
1. Берем `123`.
2. Выкидываем первую цифру: `123 - 100 = 23`.
3. Сдвигаем влево: `23 * 10 = 230`.
...
Это очень быстро, почти как просто прочитать текст один раз.
...
*На практике при хорошем выборе функции хеширования худший случай встречается крайне редко*
